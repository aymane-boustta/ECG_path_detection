...
record_list = physionetdb('incartdb');
...
path_to_save_records = '/Users/macbookair/Desktop/Thesis_ecg/Database/incartdb';
...
for i=1:length(record_list)
    ...
    command_annot = char(strcat(path_to_exes, filesep, 'rdann -r incartdb/', record_list(i), ' -a atr -v >', record_list(i), 'm.txt'));
    system (command_annot);
    command_mat_info_ = char(strcat(path_to_exes, filesep, 'wfdb2mat -r incartdb/', record_list(i), ' >', record_list(i), 'm.info'));
