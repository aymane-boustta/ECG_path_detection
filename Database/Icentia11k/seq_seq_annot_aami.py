...
random.seed(654)
def read_mitbih(filename, max_time=100, classes= [ 'N', 'S', 'V', 'Q' ], max_nlabel=100):
    def normalize(data):
    ...
    directory = '.' 
    processed_files = set()
    #empty_array = np.empty((0, 0), dtype=np.uint8)
    #samples = {'s2s_mitbih': {'seg_values': np.empty((0, 0), dtype=np.uint8), 'seg_labels': np.empty((0, 0), dtype=np.uint8)}}
    v = 1
    excluded_numbers = {8, 18, 80, 188}
    #values = np.empty((0, 0), dtype=np.uint8)
    #labels = np.empty((0, 0), dtype=np.uint8)
    for _ in range(6):
        i = random.choice([num for num in range(100) if num not in excluded_numbers])
        filename = "s2s_11K_aami_{}.mat".format(i)
        # Check if the file exists and has not been processed yet
        if os.path.exists(os.path.join(directory, filename)) and filename not in processed_files:
            print("Processing file: {}".format(filename))
            filepath = os.path.join(directory, filename)

            try:
                samples = spio.loadmat(filepath)
                ...
                if v == 1 :
                    ...
                    values = np.concatenate([values, samples[0]['seg_values']])
                    labels = np.concatenate([labels, samples[0]['seg_labels']])
...
def main():
    ...
    parser.add_argument('--data_dir', type=str, default='Test_11k/s2s_11k_aami')
    parser.add_argument('--bidirectional', type=str2bool, default=str2bool('True'))
    # parser.add_argument('--lstm_layers', type=int, default=2)
    parser.add_argument('--num_units', type=int, default=128)
    parser.add_argument('--n_oversampling', type=int, default=81000)
    parser.add_argument('--checkpoint_dir', type=str, default='checkpoints-seq2seq')
    parser.add_argument('--ckpt_name', type=str, default='seq2seq_11K.ckpt')
...
def run_program(args):
    ...
    print("Unique classes in X_train:", unique_classes)
    print('======================================')
    ratio={0:nums[0],1:n_oversampling,2:n_oversampling,3:n_oversampling}
    ...
    with tf.Session() as sess:
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            checkpoint_folders = ['/Users/macbookair/Desktop/Thesis_ecg/Database/Test_MITBIH/checkpoints-seq2seq', '/Users/macbookair/Desktop/Thesis_ecg/Database/incartdb/checkpoints-seq2seq', '/Users/macbookair/Desktop/Thesis_ecg/Database/nsrdb/checkpoints-seq2seq']
            #checkpoint_folders = ['../Test_MITBIH/checkpoints-seq2seq', '../incartdb/checkpoints-seq2seq', '../nsrdb/checkpoints-seq2seq']
