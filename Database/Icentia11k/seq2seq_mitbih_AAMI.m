...
%clc
    ...
    for j = 0:200
        tic
        base_path = '../11k/physionet.org/files/icentia11k-continuous-ecg/1.0/p00/';
        subdirectory = sprintf('p%05d', j);
        addr = fullfile(base_path, subdirectory);
        ...
        for i=1:length(Files) % Files names3signals
            ...
            % choose the first derivation 'nsig' : modify it for incartdb nsig = 2
            signal = ecgsig(nsig,:);
        ...
        save(['s2s_nsrdb_aami_', num2str(j), '.mat'], 's2s_mitbih');
        toc
        F_class
        N_class
        Q_class
