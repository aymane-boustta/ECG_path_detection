...
% Exclude files that start with 's2s'
Files = Files(~startsWith({Files.name}, 's2s'));
j = 1;
k = 1;
...
for i=1:length(Files) % Files names3signals
    ...
    [tm,ecgsig,ann,Fs,sizeEcgSig,timeEcgSig] = loadEcgSig_nsrdb([addr filesep name]);
    ...
    %tpeaks = peaks(:,7);  
    if ~isempty(peaks) && size(peaks, 2) >= 7
        tpeaks = peaks(:, 7);
    else
        disp('Peaks matrix is empty or does not have 7 columns.');
    end
...
%              end
    ...
    s2s_mitbih.seg_values = seg_values';
    s2s_mitbih.seg_labels = char(seg_labels);
    
    % featuresSeg = [featuresSeg; peakSegFeats(N_inds,:),repmat(0,length(N_inds),1)];
    
    j = 0;
    % Save the current s2s_mitbih to a file
    save(['s2s_nsrdb_aami_', num2str(k), '.mat'], 's2s_mitbih');
    k = k + 1;
    % Reset s2s_mitbih for the next batch
    s2s_mitbih = [];
    disp('smooth');
