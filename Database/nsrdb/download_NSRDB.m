...
for i=1:length(record_list)
    ...
    command_annot = char(strcat(path_to_exes, filesep, 'rdann -r nsrdb/', record_list(i), ' -a atr -v >', record_list(i), 'm.txt'));
    system (command_annot);
    command_mat_info_ = char(strcat(path_to_exes, filesep, 'wfdb2mat -r nsrdb/', record_list(i), ' >', record_list(i), 'm.info'));
